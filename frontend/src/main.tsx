...
type User={id:number;username:string;display_name:string;role:'superadmin'|'warehouse_admin'|'standard';disabled:boolean}
...
function TakeDialog({item,onClose,onDone}:{item:Product;onClose:()=>void;onDone:()=>void}){
...
function Inventory({user}:{user:User}){
  ...
  const [view,setView]=useState<'grid'|'list'>(()=>localStorage.getItem('inventory_view')==='list'?'list':'grid');
  const [take,setTake]=useState<Product|null>(null);const [edit,setEdit]=useState<Product|null>(null);const [columns,setColumns]=useState<string[]>([]);const [watched,setWatched]=useState<string[]>([]);const [mapping,setMapping]=useState<Record<string,string>>({});const [sort,setSort]=useState<SortKey>(()=>(['name','stock','location','manufacturer','model','discontinued'].includes(localStorage.getItem('inventory_sort')||'')?localStorage.getItem('inventory_sort'):'name') as SortKey);const [direction,setDirection]=useState<'asc'|'desc'>(()=>localStorage.getItem('inventory_sort_direction')==='desc'?'desc':'asc');
  ...
  const changeView=(next:'grid'|'list')=>{setView(next);localStorage.setItem('inventory_view',next)};
...
function Shell({user,onLogout}:{user:User;onLogout:()=>void}){const [page,setPage]=useState<Page>('inventory');const [open,setOpen]=useState(false);const items:{page:Page;label:string;icon:React.ReactNode}[]=[{page:'inventory',label:'Inventory',icon:<Box/>},{page:'requests',label:'Requests',icon:<ClipboardList/>},{page:'activity',label:'My activity',icon:<History/>},...(user.role!=='standard'?[{page:'ordering' as Page,label:'Ordering',icon:<ClipboardList/>},{page:'transactions' as Page,label:'Transactions',icon:<History/>},...(user.role==='superadmin'?[{page:'users' as Page,label:'Users',icon:<Users/>},{page:'settings' as Page,label:'Settings',icon:<Settings/>}]:[])]:[])];const navigate=(p:Page)=>{setPage(p);setOpen(false)};return <div className="app-shell"><button aria-label="Toggle navigation" aria-expanded={open} className="mobile-menu" onClick={()=>setOpen(!open)}><Menu/></button><aside className={open?'open':''}><div className="wordmark"><div className="brand-mark small"><Box/></div><span>Warehouse<br/><b>Inventory</b></span></div><nav>{items.map(x=><button key={x.page} className={page===x.page?'active':''} onClick={()=>navigate(x.page)}>{x.icon}<span>{x.label}</span></button>)}</nav><div className="account"><div><strong>{user.display_name}</strong><span>{roleLabel(user.role)}</span></div><button className="icon-button" title="Log out" onClick={onLogout}><LogOut/></button></div></aside>{open&&<div className="nav-scrim" onClick={()=>setOpen(false)}/>}<main className="content">{page==='inventory'&&<Inventory user={user}/>} {page==='requests'&&<RequestsPage user={user}/>} {page==='ordering'&&<Procurement/>} {page==='activity'&&<Activity/>} {page==='transactions'&&<Activity admin/>} {page==='users'&&<UsersPage/>} {page==='settings'&&<SettingsPage/>}</main>{page!=='requests'&&<button className="floating-request" onClick={()=>navigate('requests')}><PackagePlus/> Request item</button>}</div>}
