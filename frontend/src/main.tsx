...
function SettingsPage(){
  ...
  const [error,setError]=useState('');const [message,setMessage]=useState('');const [busy,setBusy]=useState(false);const mappingDirty=useRef(false);
  const load=useCallback(async()=>{try{const [next,issues,schema]=await Promise.all([api<SyncStatus>('/admin/status'),api<typeof conflicts>('/admin/sync/conflicts'),api<{columns:string[];mapping:Record<string,string>}>('/admin/columns')]);setStatus(next);setConflicts(issues);setData(current=>mappingDirty.current&&current?{...current,columns:schema.columns}:schema);setError('')}catch(e){setError((e as Error).message)}},[]);
  useEffect(()=>{load();const timer=setInterval(load,5000);return()=>clearInterval(timer)},[load]);
  const save=async()=>{try{if(data){const mapping=await api<Record<string,string>>('/admin/mapping',{method:'PUT',body:JSON.stringify({mapping:data.mapping})});mappingDirty.current=false;setData({...data,mapping})}setError('');setMessage('Mapping saved · background sync requested')}catch(e){setError((e as Error).message)}};
