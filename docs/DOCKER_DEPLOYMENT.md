...
`ghcr.io/thodgesproav/warehouseinv:1.3.2` is one multi-platform image (Linux AMD64 and ARM64). It serves the compiled website and API on port 8000 and runs inventory/email workers in the same process. SQLite and persistent files live in `/data`. No other services are required. The image runs as non-root UID/GID **10001**, has a health check, and supports a read-only root filesystem.
...
```bash
docker pull ghcr.io/thodgesproav/warehouseinv:1.3.2
docker run -d --name inventory --restart unless-stopped --stop-timeout 90 \
  ...
  ghcr.io/thodgesproav/warehouseinv:1.3.2
...
To build instead of pulling, run `docker build -t warehouse-inventory:1.3.2 .` on the target host. The published image selects Linux AMD64 or ARM64 automatically.
