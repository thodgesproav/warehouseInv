...
`WarehouseInventoryLauncher.ch5z` is the production Crestron HTML5 touch-screen project for a TSW-1060. It is permanently assigned to `http://192.168.68.7:8000` and navigates to the inventory app as a top-level page so the app's same-origin login cookie works normally. It also supplies the warehouse-panel registration token used for camera evidence capture and the panel-specific interface. Before opening the app, it enables the camera stream at 1920x1080 through the panel's documented camera reserved joins.
